...
namespace Frankstein.Common
{
    public static class EmailExtensions
    {
        public static async Task SendEmailAsync(
            MailAddress addressFrom, MailAddress addressTo, string subject, string htmlOrPlainText, bool plain,
            Action<MailMessage, Exception> onError)
        {
            using (var client = new SmtpClient())
            {
                ...
                if (plain)
                {
                    var plainView = AlternateView.CreateAlternateViewFromString(htmlOrPlainText, msg.BodyEncoding,
                        "text/plain");
